...
class IAcfConan(ConanFile):
    ...
    description = "Additional components for ACF (Application Component Framework)"
    url = "https://github.com/ImagingTools/IAcf"
    ...
    def requirements(self):
        self.requires("acfsln/[>=1.0]@gmg/stable")
    ...
    def layout(self):
        ...
        for layoutData in (self.layouts.source, self.layouts.package):
            layoutData.buildenv_info.define_path("IACFDIR", ".")

        self.layouts.build.buildenv_info.define_path("IACFDIR_BUILD", "IAcf")
    ...
    def generate(self):
        ...
        env = Environment()
        env.define("IACFDIR_BUILD", os.path.join(self.build_folder, "IAcf"))
        env.define("QTDIR", qtDir)
        env = env.vars(self, scope="build")
        env.save_script("iacfenv")
    ...
    def build(self):
        # HACK: the IAcf build is disabled for now, we only need its environment for ImtCore
        self.output.info("Skipping IAcf build")
        return
    ...
    def package_info(self):
        # Acf include dirs: "Impl", "Include", "AuxInclude/XXX", "AuxInclude/XXX/GeneratedFiles"
        iacf_auxinc_dir = os.path.join("IAcf/AuxInclude", self._include_folder_suffix())
        iacf_dirs = ["Impl", "Include"]
        iacf_auxinc_dirs = [iacf_auxinc_dir, os.path.join(iacf_auxinc_dir, "GeneratedFiles")]

        self.cpp_info.bindirs = []
        self.cpp_info.includedirs = iacf_dirs + iacf_auxinc_dirs
        self.cpp_info.libdirs = [os.path.join("IAcf/Lib", self._build_folder_suffix())]
        # HACK: the IAcf build is disabled for now, we only need its environment for ImtCore
        ...
        self.cpp.source.includedirs = iacf_dirs
        self.cpp.build.includedirs = iacf_auxinc_dirs
        ...
        if len(qt_components) > 0:
            ...
            self.cpp_info.requires = ['qt::qtCore', 'acfsln::acfsln']
        else:
            ...
            self.cpp_info.requires = ['qt::qt', 'acfsln::acfsln']
