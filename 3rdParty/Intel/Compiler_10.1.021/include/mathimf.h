...
_LIBIMF_EXT _LIBIMF_INT signbitd( _LIBIMF_DOUBLE __x );
#else
...
#define __IMFC99MACRO1ARG_ld( __x__, __func__ ) \
	(( sizeof( __x__ ) > sizeof( double )) \
	 ? __func__##l( (long double)(__x__) ) \
	 : (( sizeof( __x__ ) == sizeof( float )) \
	    ? __func__##f( (float)(__x__) ) \
...
#if defined (__APPLE__)
#define signbit( __x__ )    __IMFC99MACRO1ARG_ld( __x__, __signbit )
#else
#define signbit( __x__ )    __IMFC99MACRO1ARG( __x__, __signbit )
#endif
