...
#include "generated/moc_qtservice_win.cpp"
...
{
	QtServiceController controller(q_ptr->serviceName());
	if (!controller.isInstalled()){
		QStringList applicationArgs = args;

		QString debug = applicationArgs.join(" ");

		// remove -i account password
		applicationArgs.removeAt(1);
		applicationArgs.removeAt(1);
		applicationArgs.removeAt(1);

		// merge the rest and return full application name  + application arguments:
		return applicationArgs.join(" ");
	}
