...
void Plot3D::paintGL()
{
  ...
  glRotatef( 0.0, 0.0, 0.0, 1.0 );
	...
	double radiusY = (center-beg).length();
	double radiusX = radiusY * width() / height();
  ...
  glLoadIdentity();
	...
	{		
		if (ortho_)
    {	
      glOrtho( -radiusX, +radiusX, -radiusY, +radiusY, 0, 40 * radiusY);
    }
    else
    {	
      glFrustum( -radiusX, +radiusX, -radiusY, +radiusY, 5 * radiusY, 400 * radiusY );
    }
  }
	else
	{
  ...
  glTranslatef( xVPShift_ * 2 * radiusX , yVPShift_ * 2 * radiusY , -7 * radiusY );
